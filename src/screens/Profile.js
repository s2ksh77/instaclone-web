...
const Profile = () => {
  ...
  const [unfollowUser] = useMutation(UNFOLLOW_USER_MUTATION, {
    ...
    onCompleted: unFollowUserUpdate,
    // refetchQueries: [{ query: SEE_PROFILE_QUERY, variables: { username } }],
  });
  ...
  const getButton = (seeProfile) => {
    ...
    if (isFollowing) return <ProfileBtn onClick={unfollowUser}>Unfollow</ProfileBtn>;
  ...
  return (
    <div>
      <PageTitle
        title={loading ? 'Loading...' : `${data?.seeProfile?.username} 's Profile`}
      />
      <Header>
